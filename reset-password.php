...
<body>
    ...
    if (isset($_POST['reset'])) {
        ...
        if (mysqli_num_rows($result) > 0) {
            $sql2 = "SELECT * FROM users WHERE phone = '{$phone}' AND pet_name = '{$petname}'";
            $result2 = mysqli_query($conn, $sql2) or die("Query Failed");
            if (mysqli_num_rows($result2) > 0) {
                $sql3 = "UPDATE users SET password = '{$newpassword}' WHERE phone = '{$phone}' AND pet_name = '{$petname}'";
      ...
      window.location.href = '{$host}/login.php';
                    ...
                    alertPopup('Password reset successfully 😊!');
                }
            } else {
                alertPopup('Wrong petname 😞!');
            }
        } else {
            alertPopup('invalid user 😞!');
        }
        mysqli_close($conn);
    }
    ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                ...
                                <input class="form-control border-form-control" placeholder="john" name="petname" type="text" required>
                            </div>
                        </div>
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label class="control-label">New Password <span class="required">*</span></label>
                                <input class="form-control border-form-control" placeholder="***********" name="newpassword" type="password" required>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12 text-center">
                            <input type="submit" value="Reset" class="btn btn-success btn-lg w-100" name="reset">
                        </div>
                    </div>
                </form>
            </div>
